...
RSpec.describe Forms::AddAnotherAnswerController, type: :request do
  ...
  describe "GET #show" do
    before do
      get add_another_answer_path(mode: "preview-draft", form_id: form.id, form_slug: form.form_slug, locale:, page_slug: first_step_in_form.id)
    end
  ...
  describe "POST #save" do
    before do
      post add_another_answer_path(mode: "preview-draft", form_id: form.id, form_slug: form.form_slug, locale:, page_slug: first_step_in_form.id), params:
    end
    ...
    context "with valid params" do
      context "when adding another answer" do
        ...
        it "redirects to first page to add another" do
          expect(response).to redirect_to(form_page_path(mode: "preview-draft", form_id: form.id, form_slug: form.form_slug, locale:, page_slug: first_step_in_form.id, answer_index: 3))
        end
      end
      ...
      context "when not adding another answer" do
        ...
        it "redirects to next page" do
          expect(response).to redirect_to(form_page_path(mode: "preview-draft", form_id: form.id, form_slug: form.form_slug, locale:, page_slug: second_step_in_form.id))
        end
      end
    end
    ...
    context "with the maximum number of answers" do
      ...
      it "renders the show template with an error" do
        post add_another_answer_path(mode: "preview-draft", form_id: form.id, form_slug: form.form_slug, locale:, page_slug: first_step_in_form.id), params: { add_another_answer_input: { add_another_answer: "yes" } }
        expect(response).to render_template(:show)
  ...
  describe "redirect_if_not_repeating" do
    context "when step is not RepeatableStep" do
      it "redirects to form_page when not changing existing answer" do
        get add_another_answer_path(mode: "preview-draft", form_id: form.id, form_slug: form.form_slug, locale:, page_slug: second_step_in_form.id)
        expect(response).to redirect_to(form_page_path(mode: "preview-draft", form_id: form.id, form_slug: form.form_slug, locale:, page_slug: second_step_in_form.id))
      end

      it "redirects to form_change_answer_path when changing existing answer" do
        get change_add_another_answer_path(mode: "preview-draft", form_id: form.id, form_slug: form.form_slug, locale:, page_slug: second_step_in_form.id)
        expect(response).to redirect_to(form_change_answer_path(mode: "preview-draft", form_id: form.id, form_slug: form.form_slug, locale:, page_slug: second_step_in_form.id))
      end
    end
  end
end
