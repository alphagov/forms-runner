...
RSpec.describe Forms::RemoveAnswerController, type: :request do
  ...
  describe "GET #show" do
    before do
      get form_remove_answer_path(mode: "preview-draft", form_id: form.id, form_slug: form.form_slug, locale:, page_slug: first_step_in_form.id, answer_index: 1)
    end
  ...
  describe "DELETE #delete" do
    ...
    before do
      delete form_remove_answer_path(mode: "preview-draft", form_id: form.id, form_slug: form.form_slug, locale:, page_slug: first_step_in_form.id, answer_index: 1), params:
    end

    context "with valid params" do
      it "redirects to add another answer" do
        expect(response).to redirect_to(add_another_answer_path(mode: "preview-draft", form_id: form.id, form_slug: form.form_slug, locale:, page_slug: first_step_in_form.id))
      end
    ...
    context "when removing an answer for an optional question with no remaining answers" do
      ...
      it "redirects to the question page" do
        expect(response).to redirect_to(form_page_path(mode: "preview-draft", form_id: form.id, form_slug: form.form_slug, locale:, page_slug: first_step_in_form.id))
      end
    end
  end
end
