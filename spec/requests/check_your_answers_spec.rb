    ...
    context "with all questions answered and valid" do
      ...
      context "with preview mode on" do
        before do
          ...
          post preview_save_form_page_path(2, 2), params: { question: { text: "answer text" }, changing_existing_answer: false }
          allow(EventLogger).to receive(:log).at_least(:once)
          get preview_check_your_answers_path(2)
        end
        ...
        it "Displays a back link to the last page of the form" do
          expect(response.body).to include(preview_form_page_path(2, 2))
        end

        it "Contains a change link for each page" do
          expect(response.body).to include(preview_form_change_answer_path(2, 1))
          expect(response.body).to include(preview_form_change_answer_path(2, 2))
        end
