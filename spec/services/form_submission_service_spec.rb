...
RSpec.describe FormSubmissionService do
  let(:service) { described_class.call(logging_context:, current_context:, email_confirmation_form:, preview_mode:) }
  let(:form) do
    build(:form,
          id: 1,
          name: "Form 1",
  ...
  describe "FormSubmissionService::NotifyTemplateBodyFilter" do
    let(:notify_template_body_filter) { NotifyTemplateBodyFilter.new }
