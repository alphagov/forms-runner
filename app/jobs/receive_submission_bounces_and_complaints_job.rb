...
private
  ...
  def process_messages(sqs_client, queue_url, messages)
    messages.each do |message|
      ...
      process_bounce(ses_message_id) if ses_event_type == "Bounce"
      process_complaint(ses_message_id) if ses_event_type == "Complaint"
  ...
  def process_bounce(ses_message_id)
    ...
    Rails.logger.info("Submission email bounced - #{self.class.name}")
  ...
  def process_complaint(ses_message_id)
    submission = Submission.find_by(mail_message_id: ses_message_id)

    set_submission_logging_attributes(submission)

    Rails.logger.info("Submission email complaint received - #{self.class.name}")
  end
end
