class SubmissionS3Service
  def upload_file_to_s3(file_path, form_name, submission_reference)
    s3 = Aws::S3::Client.new(
      region: "eu-west-2",
      credentials: assume_role(submission_reference),
    )
    key_name = key_name(form_name, submission_reference)
    s3.put_object({
...
private
  ...
  def key_name(form_name, submission_reference)
    title_part = form_name.parameterize(separator: "_")
    "govuk_forms_submission_#{title_part}_#{submission_reference}.csv"
  end
end
