...
class SftpService
  def upload(file_path, form_id, timestamp, submission_reference)
    Net::SFTP.start("localhost", "test", password: "fill me in", port: 2222) do |sftp|
  ...
  def upload_using_public_key_auth(file_path, form_id, timestamp, submission_reference)
    private_key_path = "fill me in"
    Net::SFTP.start("localhost", "test", port: 2222, keys: [private_key_path]) do |sftp|
      remote_dir = remote_dir_name(form_id)
      # create_remote_dir(sftp, remote_dir)
  ...
  def remote_dir_name(form_id)
    ...
    # Directory will be stored on form to avoid having to create?
    "form_submissions_5"
  end
