  ...
  private
    ...
    def next_page_after_removing
      if @step.question.is_optional? && @step.show_answer.blank?
        form_page_path(current_form.id, current_form.form_slug, @step.page_slug, changing_existing_answer:)
      else
        add_another_answer_path(current_form.id, current_form.form_slug, @step.page_slug, changing_existing_answer:)
      end
    end
  end
end
